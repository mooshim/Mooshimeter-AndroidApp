		...
		while( currentPos < scanRecord.length ) {
			...
			switch (fieldType) {
				...
				case DATA_TYPE_MANUFACTURER_SPECIFIC_DATA:
					...
					try
					{
						// FIXME: JWhong hacked this out, I don't think this is correct
						// Just feed back the mfgData as a single blob
						//mfgId = ((scanRecord[currentPos + 1] & 0xFF) << 8) + (scanRecord[currentPos] & 0xFF);
						mfgData = extractBytes(scanRecord, currentPos, dataLength);
					}
					catch(Exception e)
					{
